...
description: Guide a person through MRI/CT imagery in a running local ScanView viewer. Use when asked what is visible, what a crosshair or colored brush mark represents, how a finding relates to nearby anatomy, which series or slice is open, or when asked to navigate, compare views, add discussion highlights, inspect metadata, or retrieve a DICOM instance for on-device analysis. Control ScanView only through its agent API and answer image questions in a terse, direct format.
---

# Guide through ScanView

Keep pixels, coordinates, and metadata on the local computer.

## Connect

1. Confirm `scanview-agent launch <folder>` is running.
2. Put its token in `SCANVIEW_AGENT_TOKEN`; never print or commit it.
3. Use `scripts/scanview_control.py` with `.venv/bin/python`.
4. Read `../../docs/AGENT-API.md` before composing raw HTTP.
