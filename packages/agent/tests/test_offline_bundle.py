...
def _inputs(tmp_path: Path) -> tuple[Path, Path]:
    return (
        _wheel(
            tmp_path / "dicom_guide-0.16.0-py3-none-any.whl",
            name="dicom-guide",
            version="0.16.0",
        ),
        _wheel(
...
def test_offline_bundle_is_exact_deterministic_and_local_only(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(first) as archive:
        ...
        assert all(name.startswith("dicom-guide-offline-0.16.0/") for name in names)
        assert all(info.date_time == (2020, 2, 2, 0, 0, 0) for info in infos)
        manifest = json.loads(
            archive.read("dicom-guide-offline-0.16.0/bundle.json")
        )
        requirements = archive.read(
            "dicom-guide-offline-0.16.0/requirements.lock"
        ).decode()
        install = archive.read("dicom-guide-offline-0.16.0/install.sh").decode()
        launch = archive.read("dicom-guide-offline-0.16.0/launch.sh").decode()
        runtime_check = archive.read(
            "dicom-guide-offline-0.16.0/runtime_check.py"
        ).decode()
    ...
    assert set(manifest["files"]) == {
        ...
        "wheels/dicom_guide-0.16.0-py3-none-any.whl",
    }
...
def test_verifier_accepts_runtime_but_rejects_tamper_and_extra_files(
    ...
    (root / "README.md").write_bytes(
        zipfile.ZipFile(_build(tmp_path, "fresh")).read(
            "dicom-guide-offline-0.16.0/README.md"
        )
    )
