...
def _inputs(tmp_path: Path) -> tuple[Path, Path]:
    return (
        _wheel(
            tmp_path / "scanview_agent-0.3.0-py3-none-any.whl",
            name="scanview-agent",
            version="0.3.0",
        ),
        _wheel(
...
def test_offline_bundle_is_exact_deterministic_and_local_only(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(first) as archive:
        ...
        assert all(name.startswith("scanview-offline-0.3.0/") for name in names)
        assert all(info.date_time == (2020, 2, 2, 0, 0, 0) for info in infos)
        manifest = json.loads(
            archive.read("scanview-offline-0.3.0/bundle.json")
        )
        requirements = archive.read(
            "scanview-offline-0.3.0/requirements.lock"
        ).decode()
        install = archive.read("scanview-offline-0.3.0/install.sh").decode()
        launch = archive.read("scanview-offline-0.3.0/launch.sh").decode()
        runtime_check = archive.read(
            "scanview-offline-0.3.0/runtime_check.py"
        ).decode()
    ...
    assert set(manifest["files"]) == {
        ...
        "wheels/scanview_agent-0.3.0-py3-none-any.whl",
    }
    ...
    assert "schema_count != 22" in runtime_check
...
def test_verifier_accepts_runtime_but_rejects_tamper_and_extra_files(
    ...
    (root / "README.md").write_bytes(
        zipfile.ZipFile(_build(tmp_path, "fresh")).read(
            "scanview-offline-0.3.0/README.md"
        )
    )
