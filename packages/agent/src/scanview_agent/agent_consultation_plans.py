...
REQUEST_ARTIFACT_TYPE = "scanview.agent-consultation-plan-request"
MEDIA_TYPE = "application/vnd.scanview.agent-consultation-plan+json"
MIN_ITEMS = 2
MAX_ITEMS = 8
...
def _catalog_index(catalog: Any) -> tuple[str, dict[str, dict[str, Any]]]:
    if (
        ...
        raise ValueError("consultation plans require a ScanView manifest v1 catalog")
    try:
    ...
    for study in catalog["studies"]:
        ...
        for series in study["series"]:
            ...
            if (
                ...
                raise ValueError("ScanView catalog contains an invalid renderable series")
...
def agent_consultation_plan_summary(
    ...
    try:
        ...
        return {
            ...
            "schema_version": SCHEMA_VERSION,
            "artifact_type": "scanview.agent-consultation-plan-summary",
            "item_count": 0,
            "modalities_present": [],
