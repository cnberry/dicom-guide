...
ARTIFACT_TYPE = "scanview.lesion-volume-comparison-review"
REQUEST_ARTIFACT_TYPE = "scanview.lesion-volume-comparison-request"
EXPECTED_FILES = {
    "comparison.json",
    "baseline-review.zip",
    "followup-review.zip",
...
ATTESTATION = (
    ...
    "comparability, and registration need. ScanView has not verified my identity or "
    "credentials."
)
LIMITATIONS = [
...
<tr><th>Reviewed volume</th><td>{comparison['baseline_volume_ml']:.6f} mL</td><td>{comparison['followup_volume_ml']:.6f} mL</td><td>{comparison['absolute_change_ml']:.6f} mL · {comparison['percent_change']:.2f}%</td></tr>
    ...
    page = f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ScanView reviewed volume comparison</title><style>
...
def _readme() -> bytes:
    return (
        "ScanView reviewed manual ROI volume comparison\n\n"
        ...
        "  scanview-agent validate-lesion-volume-comparison comparison.zip '/path/to/DICOM-root'\n\n"
