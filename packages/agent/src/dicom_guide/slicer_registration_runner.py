"""Headless 3D Slicer entry point for one DICOM Guide rigid registration job.

This file is launched by the required, version-gated Slicer application, not by the
ordinary DICOM Guide Python runtime. The host process supplies a private request path
...
def _constant_one_moving_grid(moving):
    ...
    support = slicer.mrmlScene.AddNewNodeByClass(
        "vtkMRMLLabelMapVolumeNode", "DICOM Guide moving sampling support"
    )
...
def run() -> None:
    ...
    fixed = _load_one_scalar_series(Path(request["fixed_input_dir"]), "DICOM Guide fixed")
    moving = _load_one_scalar_series(Path(request["moving_input_dir"]), "DICOM Guide moving")
    ...
    registered = slicer.mrmlScene.AddNewNodeByClass(
        "vtkMRMLScalarVolumeNode", "DICOM Guide registered moving"
    )
    transform = slicer.mrmlScene.AddNewNodeByClass(
        "vtkMRMLLinearTransformNode", "DICOM Guide moving to fixed"
    )
    ...
    registered_support = slicer.mrmlScene.AddNewNodeByClass(
        "vtkMRMLLabelMapVolumeNode", "DICOM Guide registered moving sampling support"
    )
