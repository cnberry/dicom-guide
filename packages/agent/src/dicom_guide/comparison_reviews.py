...
SELF_ATTESTATION = (
    ...
    "of my stated role; DICOM Guide has not verified my identity or credentials."
)
AMENDMENT_ATTESTATION = (
    "I attest that I intentionally supplied this amended comparison; DICOM Guide has "
    "not verified my identity or credentials."
)
RECORD_LIMITATIONS = [
    "Reviewer identity and credentials are self-asserted and are not cryptographically verified by DICOM Guide.",
...
def _initial_record(components: ReviewComponents, created_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        "implementation": {
            "name": "DICOM Guide local comparison-review assembler",
            "version": "0.1.0",
        },
        "files": {},
    }
...
<head>
  ...
  <title>DICOM Guide local comparison review</title>
  <style>
...
<body><main>
  <h1>DICOM Guide local comparison review</h1>
  <div class="banner">{status} · SELF-ATTESTED IDENTITY IS NOT CRYPTOGRAPHICALLY VERIFIED · NO RESPONSE CONCLUSION GENERATED BY DICOM_GUIDE</div>
  ...
  <footer>Original DICOM instances and the clinical medical record remain authoritative. Keep ancestor review archives to preserve parent-archive verification. Validate locally with dicom-guide before use.</footer>
</main></body></html>\n""".encode()


def _render_readme() -> bytes:
    return (
        "DICOM Guide local comparison review packet\n"
        "\n"
        ...
        "4. Validate locally: dicom-guide validate-comparison-review <archive.zip>\n"
        "\n"
        "Reviewer names, roles, organizations, checklist choices, and notes are self-asserted.\n"
        "DICOM Guide does not authenticate clinical credentials or create a digital signature.\n"
        "The numeric comparison remains unreviewed and DICOM Guide generates no response conclusion.\n"
