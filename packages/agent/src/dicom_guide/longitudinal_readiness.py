...
ARTIFACT_TYPE = "dicom-guide.longitudinal-readiness"
...
def _validate_catalog(catalog: Any) -> list[dict[str, Any]]:
    if (
        ...
        raise ValueError("DICOM Guide catalog shape is invalid")
    dicom_instances = catalog["source"].get("dicom_instances")
    if type(dicom_instances) is not int or dicom_instances < 0:
        raise ValueError("DICOM Guide catalog source count is invalid")
    ...
    for study in catalog["studies"]:
        ...
        if study["id"] in study_ids or not isinstance(study.get("series"), list):
            raise ValueError("DICOM Guide catalog study membership is invalid")
        study_ids.add(study["id"])
        date = study.get("acquisition_date")
        if date is not None and not _valid_dicom_date(date):
            raise ValueError("DICOM Guide catalog contains an invalid acquisition date")
        for series in study["series"]:
            if (
                ...
                raise ValueError("DICOM Guide catalog contains an invalid series")
            patient_context = series.get("patient_context_id")
            if patient_context is not None and not OPAQUE_IDS["patient"].fullmatch(
                str(patient_context)
            ):
                raise ValueError("DICOM Guide catalog contains an invalid patient context")
...
def _modality_readiness(
    ...
    if not records:
        ...
        if not same_context_groups:
            ...
            raise ValueError("DICOM Guide readiness state is internally inconsistent")

    if state not in MODALITY_STATES:
        raise ValueError("DICOM Guide readiness modality state is unsupported")
    return {
        "modality": modality,
        "state": state,
...
def build_longitudinal_readiness(
    ...
    *,
    ...
    try:
        ...
        raise ValueError("DICOM Guide catalog cannot produce a readiness report") from error
    excluded_ids = {
        item["series_id"]
    ...
    if not isinstance(candidates, list):
        raise ValueError("DICOM Guide candidate report is invalid")
    compact_candidates = []
