...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        prog="dicom-guide",
        epilog=(
            "Active-viewer controls: state, series, show, highlight, metadata, "
            "fetch-instance. Run `dicom-guide <control> --help` for details."
        ),
    )
    ...
    validate_measurements = commands.add_parser(
        ...
        help="Validate and summarize a local DICOM Guide measurement evidence packet",
    )
    validate_measurements.add_argument("packet", type=Path)

    validate_key_image = commands.add_parser(
        "validate-key-image",
        help="Validate a local DICOM Guide key-image archive and its integrity links",
    )
    ...
    validate_visit_packet = commands.add_parser(
        ...
        help="Validate a local DICOM Guide clinician visit packet and all integrity links",
    )
...
def main() -> None:
    if len(sys.argv) > 1 and sys.argv[1] in {
        "state", "series", "show", "highlight", "metadata", "fetch-instance"
    }:
    ...
    if (
        args.command in {"serve", "open", "launch"}
