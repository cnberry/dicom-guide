...
REQUEST_ENVIRONMENT_VARIABLE = "DICOM_GUIDE_REGISTRATION_REQUEST"
...
LIMITATIONS = [
    ...
    (
        ...
        "must match, but DICOM Guide does not authenticate the distributor or code signature."
    ),
]
...
def select_registration_sources(
    ...
    *,
    ...
    if not isinstance(catalog, dict) or catalog.get("schema_version") != "1.0.0":
        raise ValueError("registration requires a DICOM Guide manifest v1 catalog")
...
def registration_doctor(slicer_executable: Path | None = None) -> dict[str, Any]:
    ...
    if not supported_platform:
        ...
        note = (
            "Install the required local Slicer build; DICOM Guide does not download an "
            "engine or use an external processing API."
        )
    elif not network_isolation["available"]:
        note = (
            "Install a supported OS network-isolation mechanism; DICOM Guide has no "
            "unsandboxed fallback."
        )
    elif not display_runtime["available"]:
        note = (
            "Install Xvfb plus xvfb-run for a private local Linux display; DICOM Guide "
...
def run_rigid_registration(
    ...
    *,
    ...
    if not runner.is_file():
        raise ValueError("the local DICOM Guide Slicer runner is unavailable")
    ...
    with tempfile.TemporaryDirectory(
        prefix="dicom-guide-registration-", dir=temporary_parent
    ) as temporary_name:
        ...
        if hash_file(staged_runner) != runner_sha256:
            raise ValueError("the local DICOM Guide Slicer runner changed while staging")
        ...
        if (
            ...
            raise ValueError("the local DICOM Guide Slicer runner changed during preflight")
        ...
        if (
            ...
            raise ValueError("the local DICOM Guide Slicer runner changed during registration")
...
def registration_bundle_errors(directory: Path) -> list[str]:
    ...
    if (
        ...
        != {
            ...
            "runner_sha256",
            "engine_identity",
            "external_api_requested_by_dicom_guide",
            "network_isolation",
        }
        ...
        or algorithm.get("external_api_requested_by_dicom_guide") is not False
