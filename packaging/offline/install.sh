...
if ! command -v "$python_command" >/dev/null 2>&1; then
  echo "ScanView requires Python 3.11 or newer. Set SCANVIEW_PYTHON to its executable." >&2
  exit 1
fi

"$python_command" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 11) else 1)' || {
  echo "ScanView requires Python 3.11 or newer." >&2
  exit 1
}
...
if [ -e "$runtime_dir" ] || [ -L "$runtime_dir" ]; then
  echo "A ScanView runtime already exists in this bundle; use it or extract a fresh bundle." >&2
  exit 1
fi
